...
const (
	twepoch        = int64(1659603739)                //开始时间截 (2022-08-04 17:02:19)
	workeridBits   = uint(8)                          //机器id所占的位数
...
func init() {
	...
	//mId, _ := strconv.ParseInt(os.Getenv("machine_id"), 10, 64)
	mId, _ := machineId()
	//fmt.Printf("%d", mId)
	GlobalSnowflake, _ = New(mId)
}
...
func machineId() (int64, error) {
	...
	for _, iface := range ifaces {
		...
		for _, addr := range addrs {
			...
			ret.SetBytes(ip.To4())
			ipInt := ret.Int64()
			return int64(byte(ipInt)), nil
		}
	}
	return 0, errors.New("connected to the network?")
}
