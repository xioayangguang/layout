...
func (l *Logger) Channel(channel string) *zap.Logger {
	if logger, ok := loggerMap[channel]; ok {
		return logger
	} else {
		var channelLogger = initZap(l.Viper, channel)
...
func initZap(conf *viper.Viper, channel string) *zap.Logger {
	// 日志地址 "out.log" 自定义
	...
	hook := lumberjack.Logger{
		Filename:   fmt.Sprintf("%s/%s/%s/log.log", lp, channel, time.Now().Format("2006-01-02")), // 日志文件路径
	...
	errF, _ := os.Create(fmt.Sprintf("%s.err.log", lp))
	c2 := zapcore.NewCore(encoder, zapcore.AddSync(errF), zap.ErrorLevel)
	core = zapcore.NewTee(core, c2)
