		...
		for range ticker.C {
			...
			if monitorState {
				...
				if v.UsedPercent >= 80 || cpuPercent[0] >= 80 {
					...
					memFile, err = os.OpenFile(fmt.Sprintf("./log/pprof/%s-mem%v.pprof", time.Now().Format("01-02-15-4-5"), int(v.UsedPercent)), os.O_CREATE, 0666)
					if err != nil {
						return
					}
			...
			if !monitorState {
				...
				if monitorStartTime+30 < time.Now().Unix() {
